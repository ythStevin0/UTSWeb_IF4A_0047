...
<div class="admin-container">
    <div class="admin-page-header">
        <h2 class="admin-page-title"><span class="title-slash">//</span> DAFTAR NEWS</h2>
        <a href="create_news.php" class="admin-btn admin-btn-success"><i class="bi bi-plus-lg"></i> TAMBAH NEWS</a>
    </div>

    <div class="admin-card">
        <div class="admin-card-header">
            <span class="admin-card-header-tag">DATA</span>
            <span class="admin-card-header-title">News Table</span>
        </div>
                ...
                <tbody>
                    <?php if($result->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td><span class="admin-badge"><?= htmlspecialchars($row['tags']) ?></span></td>
                            <td>
                                <div class="actions">
                                    <a href="edit_news.php?id=<?= $row['id'] ?>" class="admin-btn admin-btn-primary admin-btn-sm">EDIT</a>
                                    <a href="delete_news.php?id=<?= $row['id'] ?>" class="admin-btn admin-btn-danger admin-btn-sm" onclick="return confirm('Hapus news ini?');">HAPUS</a>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr class="empty-row">
                            <td colspan="6">Belum ada data news.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
